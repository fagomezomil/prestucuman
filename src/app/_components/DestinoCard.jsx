    ...
    <div>
      {circuito.destinos.map((destino, index) => (
        <div key={index} className="">
          <h3 className={`font-700 rounded-md bg-${destino.color}-1 text-white text-3xl w-fit px-4 my-2`}>{destino.nombre}</h3>
          <div className="grid grid-cols-5 gap-4">
            {destino.productos.map((producto, index) => (
              <div key={index} className="border border-neutral-100 w-fit h-fit pb-4 rounded-md relative">
                ...
                <Image src={producto.imagen} alt={`imagen ${index}`} width={250} height={180} className="w-full rounded-t-md" />
                <h4 className="text-[20px] font-700 leading-[19px] px-3 pt-3 uppercase text-neutral-700">{producto.nombre}</h4>
                <div className="flex flex-row gap-2 my-2 ml-3">
                  {producto.categorias.map((categoria, index) => (
                    <p key={index} className="rounded-md px-2 py-1 bg-[#a5a58a] text-white text-[16px] font-500">{categoria}</p>
                  ))}
                </div>
