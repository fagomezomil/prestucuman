...
duration: 1,
  ...
  return (
    <div ref={containerRef} className='bg-[#ADD29E] bg-[url(/images/textura-calchaqui.svg)] -z-20 h-screen overflow-hidden flex items-end justify-center relative'>
      {/* <Image src={TexturaCalchaqui} ref={texturaCalRef} alt="textura" priority className="w-screen opacity-15 -z-10" /> */}
      <div>
        <div ref={montanaRef} className="z-10 absolute bottom-[-400px] left-0 w-full " >
          <Image src={Montana} alt="Logo" className="w-screen" priority />
        </div>
      </div>
      <svg ref={circuloRef} width="500" height="500" className="absolute bottom-[-600px] -right-14">
        <circle cx="250" cy="250" r="250" fill="#db5723" className="bg-[]"/>
      </svg>
      <Image ref={tucumanLogoRef} src={TucumanLogo} alt="Logo" priority className="absolute -z-10 bottom-[-400px]" />
    </div>
  )
}
